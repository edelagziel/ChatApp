...
export function ChatItem({chatName,msg,id}:{chatName:string,msg:string,id:number})
{
  const navigate=useNavigate();
    return(
      <Box component={Button} onClick={()=>navigate(`/chat/${id}`)}  padding={2} display={"flex"} flexDirection={"column"} alignItems={"flex-start"} bgcolor="#90ed8c32" color="text.primary"  textTransform="none" borderRadius={2} fullWidth>
